use std::{fmt, sync::{Condvar, Arc, Mutex}};
use threadpool::ThreadPool;
use crate::{
    node::{ChangeObserver, State, Context},
...
impl ExecutionHibernator {
    ...
    fn sleep_if_possible(&mut self, controller: Arc<Mutex<ExecutionController>>) {
        ...
        controller.lock().unwrap().set_state(ExecutorState::Sleeping);
        self.sleep();
...
pub trait Executor {
    ...
    fn run<S>(&mut self, flow :Flow, scheduler: S) -> Result<()>  where S : Scheduler + std::marker::Send;
...
impl MultiThreadedExecutor {
    ...
    pub fn new(num_threads: usize, context: State<Context>) -> Self{
        ...
        context.0.lock().unwrap().set_observer(res.hibernator.clone());

        res 
    }

    fn run_update_loop<S>(&mut self, mut flow: &Flow, mut scheduler: S) 
    where S : Scheduler {

        self.controller.lock().unwrap().set_state(ExecutorState::Running);
        ...
        while !self.controller.lock().unwrap().cancellation_requested()  {
            ...
            self.hibernator.lock().unwrap().sleep_if_possible(self.controller.clone());
        }

        self.controller.lock().unwrap().set_state(ExecutorState::Ready);
...
impl Executor for MultiThreadedExecutor{
    ...
    fn run<S>(&mut self, mut flow: Flow, mut scheduler: S) -> Result<()> 
    where S : Scheduler + std::marker::Send {
